...
internal sealed interface BiliPaiNavKey : NavKey {
    ...
    /** 感谢名单：列出本工程用到的全部开源项目。 */
    @Serializable
    data object Credits : BiliPaiNavKey {
        override val routeBase: String = "credits"
    }
    ...
    /** 追番（本地收藏 + animeko 搜索）。 */
    @Serializable
    data object AniFollow : BiliPaiNavKey {
        override val routeBase: String = "ani_follow"
    }

    /** 追番历史。 */
    @Serializable
    data object AniHistory : BiliPaiNavKey {
        override val routeBase: String = "ani_history"
    }

    /**
     * 番剧设置。
     *
     * ★ 完全由番剧模块自己实现与存储, 与主 App 设置(SettingsManager)互不关联。
     */
    @Serializable
    data object AniSettings : BiliPaiNavKey {
        override val routeBase: String = "ani_settings"
    }

    /** 番剧详情（照搬 animeko 版式）。 */
    @Serializable
    data class AniDetail(
        val subjectId: Long
    ) : BiliPaiNavKey {
        override val routeBase: String = "ani_detail"
    }

    /** 番剧播放器（独立于站内播放器，含换源 + 弹幕）。 */
    @Serializable
    data class AniPlayer(
        val subjectId: Long,
        val episodeId: Long,
        val episodeSort: Double = 0.0
    ) : BiliPaiNavKey {
        override val routeBase: String = "ani_player"
    }
