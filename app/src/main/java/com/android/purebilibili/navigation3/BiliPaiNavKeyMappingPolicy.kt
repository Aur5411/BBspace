...
internal fun BiliPaiNavKey.toLegacyRoute(): String {
    return when (this) {
        ...
        BiliPaiNavKey.Credits -> ScreenRoutes.Credits.route
        ...
        // ─────────────── 番剧（animeko 数据源）───────────────
        BiliPaiNavKey.AniFollow -> ScreenRoutes.AniFollow.route
        BiliPaiNavKey.AniHistory -> ScreenRoutes.AniHistory.route
        BiliPaiNavKey.AniSettings -> ScreenRoutes.AniSettings.route
        is BiliPaiNavKey.AniDetail -> ScreenRoutes.AniDetail.createRoute(subjectId)
        is BiliPaiNavKey.AniPlayer -> ScreenRoutes.AniPlayer.createRoute(
            subjectId = subjectId,
            episodeId = episodeId,
...
internal fun legacyRouteToBiliPaiNavKey(route: String?): BiliPaiNavKey {
    ...
    return when {
        ...
        normalized == ScreenRoutes.Credits.route -> BiliPaiNavKey.Credits
        ...
        // ─────────────── 番剧（animeko 数据源）───────────────
        // 底部「番剧」主页面路由，与「追番」列表页是同一个页面。
        normalized == ScreenRoutes.Anime.route -> BiliPaiNavKey.AniFollow
        normalized == ScreenRoutes.AniFollow.route -> BiliPaiNavKey.AniFollow
        normalized == ScreenRoutes.AniHistory.route -> BiliPaiNavKey.AniHistory
        normalized == ScreenRoutes.AniSettings.route -> BiliPaiNavKey.AniSettings
        segments.firstOrNull() == "ani_detail" && segments.size >= 2 -> {
            BiliPaiNavKey.AniDetail(subjectId = segments[1].toLongOrNull() ?: 0L)
        }
        segments.firstOrNull() == "ani_player" && segments.size >= 3 -> {
            BiliPaiNavKey.AniPlayer(
                subjectId = segments[1].toLongOrNull() ?: 0L,
                episodeId = segments[2].toLongOrNull() ?: 0L,
                episodeSort = query["sort"]?.toDoubleOrNull() ?: 0.0
            )
        }
