...
internal fun NavEntryBuilder.biliPaiNavEntries(
    ...
    entry<BiliPaiNavKey.Credits>(swipeDismiss = swipeBackDirection, content = content)
    ...
    // ─────────────── 番剧（animeko 数据源）───────────────
    entry<BiliPaiNavKey.AniFollow>(swipeDismiss = swipeBackDirection, content = content)
    entry<BiliPaiNavKey.AniHistory>(swipeDismiss = swipeBackDirection, content = content)
    entry<BiliPaiNavKey.AniSettings>(swipeDismiss = swipeBackDirection, content = content)
    entry<BiliPaiNavKey.AniDetail>(swipeDismiss = NavSwipeDirection.None, content = content)
    entry<BiliPaiNavKey.AniPlayer>(
        transition = predictiveBackExcludedTransition,
        swipeDismiss = NavSwipeDirection.None,
        content = content,
    )
