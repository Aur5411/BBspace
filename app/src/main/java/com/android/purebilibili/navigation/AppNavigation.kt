...
fun AppNavigation(
    ...
    // 按需求：取消首次启动的「使用须知」安全提示门禁，冷启动直接进主页。
    // 引导页仍然保留为可达路由（设置里的「重播引导」可进入）。
    val agreementRequired = false
    ...
    CompositionLocalProvider(
        ...
        // 最近一次从服务端拿到的原始计数(用于推进基线)
        var rawDynamicUpdateCount by remember { mutableIntStateOf(0) }
        LaunchedEffect(currentBottomNavItem, dynamicUnreadPollingEnabled) {
            if (!dynamicUnreadPollingEnabled || currentBottomNavItem == BottomNavItem.DYNAMIC) {
                // ★ 进入动态页即「已确认」: 把当前计数记为基线, 红点不再复现
                //   (服务端未读数常包含 App 内看不到的内容, 直接显示会永远消不掉)
                if (rawDynamicUpdateCount > 0) {
                    com.android.purebilibili.core.util.BadgeBaselineStore.acknowledge(
                        context,
                        com.android.purebilibili.core.util.BadgeBaselineStore.KEY_DYNAMIC,
                        rawDynamicUpdateCount,
                    )
                }
            ...
            while (true) {
                ...
                com.android.purebilibili.data.repository.DynamicRepository.getDynamicUpdateCount(
                    ...
                    .onSuccess { count ->
                        rawDynamicUpdateCount = count
                        dynamicUnreadCount =
                            com.android.purebilibili.core.util.BadgeBaselineStore.displayCount(
                                context,
                                com.android.purebilibili.core.util.BadgeBaselineStore.KEY_DYNAMIC,
                                count,
                            )
                    ...
                    onSwitch = { mid ->
                        coroutineScope.launch(Dispatchers.IO) {
                            ...
                            withContext(Dispatchers.Main.immediate) {
                                ...
                                com.android.purebilibili.core.refresh.AccountRefreshBus
            ...
            Row(modifier = Modifier.fillMaxSize()) {
                ...
                fun bottomPagerNavKeyForItem(item: BottomNavItem): BiliPaiNavKey {
                    return when (item) {
                        ...
                        // 底部「番剧」主页面复用「追番」页面（animeko 数据源，零登录依赖）。
                        BottomNavItem.ANIME -> BiliPaiNavKey.AniFollow
                    ...
                    when (resolveBiliPaiNavEntryContentRole(key)) {
                        ...
                        BiliPaiNavEntryContentRole.PROFILE -> {
                            ...
                            ProfileScreen(
                                ...
                                onVideoClick = { bvid -> navigateToVideoInNavigation3(bvid, 0L, "") },
                                // 番剧小功能区（animeko 数据源，与站内追番隔离）
                                // ★ v4.2.0: 番剧缓存已移除; 「观看历史」= 追番之外的新入口
                                onAniFollowClick = { pushNavigation3Key(BiliPaiNavKey.AniFollow) },
                                // ★ 入口已由「追番历史」改为「追番」: 打开追番页(二级形态, 显示收藏列表)
                                onAniHistoryClick = { pushNavigation3Key(BiliPaiNavKey.AniFollow) },
                                onAniWatchHistoryClick = { pushNavigation3Key(BiliPaiNavKey.AniHistory) },
                        ...
                        BiliPaiNavEntryContentRole.SETTINGS ->
                            SettingsTabletEntry {
                                SettingsScreen(
                                    ...
                                    onCreditsClick = { pushNavigation3Key(BiliPaiNavKey.Credits) },
                                    ...
                                    onAnimeSettingsClick = { pushNavigation3Key(BiliPaiNavKey.AniSettings) },
                        ...
                        BiliPaiNavEntryContentRole.CREDITS ->
                            SettingsTabletEntry {
                                com.android.purebilibili.feature.settings.CreditsScreen(
                                    onBack = { performSystemBackAction() }
                                )
                            }
                        ...
                        // ─────────────── 番剧（animeko 数据源，不走 B 站任何校验）───────────────
                        BiliPaiNavEntryContentRole.ANI_FOLLOW ->
                            com.android.purebilibili.feature.anime.AnimeFollowScreen(
                                onBack = { performSystemBackAction() },
                                onSubjectClick = { subjectId ->
                                    pushNavigation3Key(BiliPaiNavKey.AniDetail(subjectId = subjectId))
                                },
                                onSettingsClick = { pushNavigation3Key(BiliPaiNavKey.AniSettings) },
                                // 作为底部「番剧」主页面时是顶层页面，不显示返回键；从「我的」二级进入时保留返回键。
                                isMainPage = isBottomPagerHosted,
                            )
                        BiliPaiNavEntryContentRole.ANI_HISTORY ->
                            com.android.purebilibili.feature.anime.AnimeHistoryScreen(
                                onBack = { performSystemBackAction() },
                                onSubjectClick = { subjectId, episodeId ->
                                    pushNavigation3Key(
                                        BiliPaiNavKey.AniPlayer(
                                            subjectId = subjectId,
                                            episodeId = episodeId
                                        )
                                    )
                        ...
                        BiliPaiNavEntryContentRole.ANI_SETTINGS ->
                            com.android.purebilibili.feature.anime.AnimeSettingsScreen(
                                onBack = { performSystemBackAction() }
                            )
                        BiliPaiNavEntryContentRole.ANI_DETAIL -> {
                            val aniDetailKey = key as BiliPaiNavKey.AniDetail
                            com.android.purebilibili.feature.anime.AnimeDetailScreen(
                                subjectId = aniDetailKey.subjectId,
                                onBack = { performSystemBackAction() },
                                onPlay = { detailSubjectId, episodeId ->
                                    pushNavigation3Key(
                                        BiliPaiNavKey.AniPlayer(
                        ...
                        BiliPaiNavEntryContentRole.ANI_PLAYER -> {
                            val aniPlayerKey = key as BiliPaiNavKey.AniPlayer
                            com.android.purebilibili.feature.anime.AnimePlayerScreen(
                                subjectId = aniPlayerKey.subjectId,
                                episodeId = aniPlayerKey.episodeId,
                                onBack = { performSystemBackAction() },
                                onSettingsClick = { pushNavigation3Key(BiliPaiNavKey.AniSettings) },
                            )
                        }
