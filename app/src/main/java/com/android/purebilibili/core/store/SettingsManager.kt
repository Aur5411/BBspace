...
import com.android.purebilibili.core.ui.performance.HighRefreshRateTarget
...
data class HomeSettings(
    ...
    // 默认右上角为「消息」（信封样式），符合 BB空间 首页「搜索 + 消息」的布局要求
    val homeTopRightAction: HomeTopRightAction = HomeTopRightAction.INBOX,
    ...
    val cardTransitionEnabled: Boolean = false,   //  ★ 应用户要求关闭卡片过渡动画(主页切换更稳)
...
data class AppNavigationSettings(
    ...
    val orderedVisibleTabIds: List<String> = listOf("HOME", "DYNAMIC", "PROFILE"),
    ...
    val miuixTransitionBlurEnabled: Boolean = false,
...
internal fun resolveListenVideoBottomTabMigration(
    ...
    // BB空间：底栏只保留「首页 / 动态 / 我的」，不再自动插入「听视频」；
    // 旧安装残留的 LISTEN_VIDEO 也在此清理掉。
    val sanitizedOrder = order.filter { it != "LISTEN_VIDEO" }
    val sanitizedVisible = visible.filterTo(mutableSetOf()) { it != "LISTEN_VIDEO" }
    return BottomTabMigrationResult(
        order = sanitizedOrder,
        visible = sanitizedVisible,
        markComplete = sanitizedOrder != order || sanitizedVisible != visible || !migrationComplete
    )
}
...
object SettingsManager {
    ...
    private const val DEFAULT_TOP_TAB_ORDER = "LIVE,RECOMMEND,POPULAR"
    private const val DEFAULT_TOP_TAB_VISIBLE = "LIVE,RECOMMEND,POPULAR"
    /** 首页顶部 dock 标签数量上限。 */
    const val MAX_TOP_TABS = 5
    /** 已下线但仍可能残留在旧安装持久化数据中的顶部标签 id。 */
    private const val RETIRED_TOP_TAB_ANIME = "ANIME"
    ...
    // 底栏严格按「首页 / 动态 / 番剧 / 我的」四个主页面呈现（与 BiliPai 默认的 5 项不同）
    private const val DEFAULT_BOTTOM_BAR_ORDER = "HOME,DYNAMIC,ANIME,PROFILE"
    private const val DEFAULT_BOTTOM_BAR_VISIBLE_TABS = "HOME,DYNAMIC,ANIME,PROFILE"
    /** BB空间：底栏固定呈现的四项主页面，其余历史 tab 一律过滤。 */
    private val BBSPACE_BOTTOM_BAR_TABS = listOf("HOME", "DYNAMIC", "ANIME", "PROFILE")
    // 评论默认排序（2=最新,3=最热）
    ...
    internal fun mapHomeSettingsFromPreferences(preferences: Preferences): HomeSettings {
        ...
        return HomeSettings(
            ...
            homeTopRightAction = HomeTopRightAction.fromValue(
                preferences[KEY_HOME_TOP_RIGHT_ACTION] ?: HomeTopRightAction.INBOX.value
            ),
    ...
    internal fun mapHomeTopTabSettingsFromPreferences(preferences: Preferences): HomeTopTabSettings {
        // BB空间：顶部「追番」标签已下线（番剧改由底部「我的」入口承载）。
        // 旧安装的持久化值里可能残留 ANIME，这里直接过滤，避免出现无内容的空标签。
        val orderIds = (preferences[KEY_TOP_TAB_ORDER] ?: DEFAULT_TOP_TAB_ORDER)
            .split(",")
            .filter { it.isNotBlank() && it != RETIRED_TOP_TAB_ANIME }
    ...
    internal fun mapPlayerInteractionSettingsFromPreferences(
        ...
        return PlayerInteractionSettings(
            ...
            // ★ 应用户要求: 进入视频不再自动全屏(播放就绪也不再自动切换全屏)
    ...
    private const val HOME_VISUAL_DEFAULTS_VERSION = 6
    ...
    suspend fun ensureHomeVisualDefaults(context: Context) {
        var resetPlaybackDefaultsToOff = false
        context.settingsDataStore.edit { preferences ->
            ...
            if (currentVersion < HOME_VISUAL_DEFAULTS_VERSION) {
                ...
                // v5：底栏改为「首页 / 动态 / 番剧 / 我的」四个主页面。
                // 旧安装持久化的 order/visible 里不含 ANIME，必须重写一次补齐，
                // 否则升级后底部看不到「番剧」入口。
                // 同时把播放设置里的「后台播放」「视频小横条」显式置为关闭
                // （这两项原先默认开启）。
                if (currentVersion < 5) {
                    preferences[KEY_BOTTOM_BAR_ORDER] = DEFAULT_BOTTOM_BAR_ORDER
                    preferences[KEY_BOTTOM_BAR_VISIBLE_TABS] = DEFAULT_BOTTOM_BAR_VISIBLE_TABS
                    preferences[KEY_BACKGROUND_PLAYBACK_ENABLED] = false
                    preferences[KEY_AUDIO_NOW_PLAYING_BAR_ENABLED] = false
                    resetPlaybackDefaultsToOff = true
                }
                // v6：默认关闭「返回过渡模糊」。
                // 它会在每次卡片进/退过渡时对下层整页做实时景深模糊重录（每帧一次全屏重录），
                // 中端机上必然掉帧 —— 本仓库里甚至专门有个掉帧采样器只盯这条路径。
                // 过渡动画与形变都保留，只是不再叠加实时模糊；
                // 想恢复可在「外观与主题 → 动画设置 → 返回过渡模糊」重新打开。
                if (currentVersion < 6) {
                    preferences[KEY_MIUIX_TRANSITION_BLUR_ENABLED] = false
                }
                preferences[KEY_NAVIGATION_ICON_CROSS_SCALE_ENABLED] = true
                preferences[KEY_HOME_VISUAL_DEFAULTS_VERSION] = HOME_VISUAL_DEFAULTS_VERSION
            }
        }
        // 这两项另有 SharedPreferences("mini_player") 镜像（存在同步读取路径），
        // 只改 DataStore 不生效，必须同步写镜像。
        if (resetPlaybackDefaultsToOff) {
            context.getSharedPreferences("mini_player", Context.MODE_PRIVATE)
                .edit()
                .putBoolean("background_playback_enabled", false)
                .putBoolean("audio_now_playing_bar_enabled", false)
                .apply()
        }
    }
    ...
    fun getBackgroundPlaybackEnabled(context: Context): Flow<Boolean> = context.settingsDataStore.data
        .map { preferences -> preferences[KEY_BACKGROUND_PLAYBACK_ENABLED] ?: false }
    ...
    fun getAudioNowPlayingBarEnabled(context: Context): Flow<Boolean> = context.settingsDataStore.data
        .map { preferences -> preferences[KEY_AUDIO_NOW_PLAYING_BAR_ENABLED] ?: false }
        .onEach { value ->
            context.getSharedPreferences("mini_player", Context.MODE_PRIVATE)
                .edit()
    ...
    fun getAudioNowPlayingBarEnabledSync(context: Context): Boolean {
        return context.getSharedPreferences("mini_player", Context.MODE_PRIVATE)
            .getBoolean("audio_now_playing_bar_enabled", false)
    }
    ...
    // 全局高刷新率（120Hz 适配）。存的是 HighRefreshRateTarget.storageValue。
    private val KEY_HIGH_REFRESH_RATE_TARGET = stringPreferencesKey("high_refresh_rate_target")
    ...
    internal fun mapAppNavigationSettingsFromPreferences(
        ...
        // BB空间：底栏只保留「首页 / 动态 / 我的」三项。旧安装（或 BiliPai 默认值）
        // 的持久化数据里可能残留 HISTORY / LISTEN_VIDEO 等已下线 tab，
        // 这里统一收敛，避免底栏又冒出「听视频」等非需求项。
        ...
        return AppNavigationSettings(
            ...
            // 默认关闭：该开关会让卡片进/退过渡每帧对下层整页做实时景深模糊重录，
            // 是中端机掉帧的主因。过渡动画本身保留，只是不叠加实时模糊。
            miuixTransitionBlurEnabled = preferences[KEY_MIUIX_TRANSITION_BLUR_ENABLED] ?: false,
    ...
    /**
     * 读取全局高刷档位。默认 [HighRefreshRateTarget.MAX]（满血高刷），
     * 不支持高刷的设备上会在应用层判定后自动不生效。
     */
    fun getHighRefreshRateTarget(context: Context): Flow<HighRefreshRateTarget> =
        context.settingsDataStore.data
            .map { preferences ->
                HighRefreshRateTarget.fromStorage(preferences[KEY_HIGH_REFRESH_RATE_TARGET])
            }

    /**
     * 同步读取高刷档位。仅用于启动早期（Compose 还没拿到 Flow）时决定窗口参数；
     * 读失败一律退回默认值，不抛异常。
     */
    fun getHighRefreshRateTargetSync(context: Context): HighRefreshRateTarget {
        val raw = runCatching {
            kotlinx.coroutines.runBlocking {
                context.settingsDataStore.data.first()[KEY_HIGH_REFRESH_RATE_TARGET]
            }
        }.getOrNull()
        return HighRefreshRateTarget.fromStorage(raw)
    }

    suspend fun setHighRefreshRateTarget(context: Context, target: HighRefreshRateTarget) {
        context.settingsDataStore.edit { preferences ->
            preferences[KEY_HIGH_REFRESH_RATE_TARGET] = target.storageValue
        }
    }
