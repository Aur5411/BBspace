...
object VideoRepository {
     ...
     * 下载专用播放地址获取（宽容档位语义）。
     *
     * 下载不要求服务端返回「精确」的目标档位轨道：当账号没有目标档位权限
     * （1080P+/1080P60/4K/HDR 等需要大会员）时，EXPLICIT 严格请求会把整条
     * fetch 链路全部拒绝，最终表现为「无法获取下载地址」。这里按
     * PLAYBACK_TRANSITION 语义请求，服务端就近降级仍接受，由 getBestVideo
     * 在返回的 DASH 轨道里选最佳可用档位。
     */
    suspend fun getPlayUrlDataForDownload(bvid: String, cid: Long, qn: Int): PlayUrlData? = withContext(Dispatchers.IO) {
        fetchPlayUrlRecursive(
            bvid = bvid,
            cid = cid,
            targetQn = qn,
