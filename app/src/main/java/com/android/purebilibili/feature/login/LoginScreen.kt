...
enum class LoginMethod {
    /** 手机号登录：App 原生表单样式，提交后由 B 站官方网页登录完成鉴权并回读 Cookie。 */
    SMS,
    /** 密码登录：App 原生表单样式，提交后由 B 站官方网页登录完成鉴权并回读 Cookie。 */
    PASSWORD,
    /**
     * App 内 TV 二维码扫码登录。已按需求下线，入口不再展示。
     * 保留枚举定义，避免 `when(selectedMethod)` 穷尽分支失效。
     */
    TV_QR,
    OFFICIAL_TV_SCAN,
    /** Cookie 导入。已按需求下线，入口不再展示。 */
    COOKIE_IMPORT,
    BILIPAI_TRANSFER
}
...
// 按需求去除扫码登录：App 内的 TV 二维码入口不再对外提供。
private const val ENABLE_TV_QR = false
// 按需求去除 Cookie 导入入口，登录页只保留手机号登录与密码登录。
private const val ENABLE_COOKIE_IMPORT = false

/**
 * 登录页对外暴露的入口：手机号登录、密码登录。
 * 两者都是 App 原生表单外观，实际由内置的 B 站官方网页登录完成鉴权。
 */
internal fun resolveAvailableLoginMethods(): List<LoginMethod> = listOf(
    LoginMethod.SMS,
...
internal fun resolveWebLoginReason(): String {
    return "登录将跳转至 B 站官方登录页完成鉴权，登录成功后自动返回本应用。"
}
...
fun LoginScreen(
    ...
    var selectedMethod by rememberSaveable { mutableStateOf(LoginMethod.SMS) }
    var transferVisible by rememberSaveable { mutableStateOf(false) }
    var webLoginVisible by rememberSaveable { mutableStateOf(false) }
    ...
    LaunchedEffect(selectedMethod) {
        if (selectedMethod == LoginMethod.BILIPAI_TRANSFER && !ENABLE_BILIPAI_TRANSFER) {
            selectedMethod = LoginMethod.SMS
            return@LaunchedEffect
        }
        // 扫码入口已下线：若历史状态恢复出 TV_QR，统一回落到手机号登录。
        if (selectedMethod == LoginMethod.TV_QR && !ENABLE_TV_QR) {
            selectedMethod = LoginMethod.SMS
            return@LaunchedEffect
        }
        // Cookie 导入入口已下线。
        if (selectedMethod == LoginMethod.COOKIE_IMPORT && !ENABLE_COOKIE_IMPORT) {
            selectedMethod = LoginMethod.SMS
            return@LaunchedEffect
        }
        captchaRequest = null
    ...
    // 用 Box 承载，保证内置网页登录页能整屏覆盖在登录方式页之上。
    ...
    LoginPage(
        ...
        onAuthorizeHighQuality = { webLoginVisible = true },
        onPrepareRiskSms = viewModel::prepareRiskSmsCaptcha,
        onVerifyRiskSms = viewModel::verifyRiskSmsCode,
        onOpenWebLogin = { webLoginVisible = true },
    )
    if (webLoginVisible) {
        WebLoginContent(
            onCookiesReady = { cookies ->
                webLoginVisible = false
                viewModel.loginByImportedCookieValues(cookies)
            },
            onClose = { webLoginVisible = false },
        )
    }
    if (transferVisible) {
        ...
        AppScaffold(
            ...
            Box(
                ...
                LazyColumn(
                    ...
                    item {
                        when (selectedMethod) {
                            // 手机号登录 / 密码登录：保持 App 原生版的呈现方式，
                            // 实际鉴权统一交给内置的 B 站官方登录页完成，再回读 Cookie。
                            LoginMethod.SMS -> WebLoginEntryContent(
                                title = "手机号登录",
                                description = "输入手机号获取验证码完成登录。" +
                                    "验证码校验与风控全部由 B 站官方登录页处理，" +
                                    "登录成功后会自动返回本应用。",
                                actionLabel = "开始手机号登录",
                                onOpenWebLogin = onOpenWebLogin,
                            )
                            LoginMethod.PASSWORD -> WebLoginEntryContent(
                                title = "密码登录",
                                description = "使用 B 站账号密码登录。" +
                                    "密码提交与安全验证全部在 B 站官方登录页完成，" +
                                    "登录成功后会自动返回本应用。",
                                actionLabel = "开始密码登录",
                                onOpenWebLogin = onOpenWebLogin,
                            )
 ...
 * 手机号 / 密码登录的统一入口卡片。
 *
 * 保持 App 自己的卡片样式，点击后进入内置的 B 站官方登录页完成鉴权：
 * 验证码下发、密码提交与风控都在站点内处理，客户端只回读登录后的 Cookie。
 */
@Composable
private fun WebLoginEntryContent(
    title: String,
    description: String,
    actionLabel: String,
    onOpenWebLogin: () -> Unit,
    ...
    LoginFormCard(title = title, modifier = modifier) {
        AppText(
            text = description,
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        ...
        AppText(
            text = "登录过程在应用内的官方页面中完成，不会跳转到外部浏览器。",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        AppButton(onClick = onOpenWebLogin, modifier = Modifier.fillMaxWidth()) {
            AppText(actionLabel)
        }
    }
}
