...
internal fun VideoDetailHighRefreshRateEffect(
    ...
    // 统一走全局高刷策略：用户在设置里选的档位（跟随系统 / 90 / 120 / 最高可用）
    // 在视频页同样生效，不再单独硬编码「≥90 取最高」。
    // MainActivity 层已经做过一次全局声明，这里再声明一次是为了在视频页
    // 被系统降频后（如切后台回来、系统省电策略介入）重新顶回去。
    val context = activity ?: return
    val target by SettingsManager.getHighRefreshRateTarget(context)
