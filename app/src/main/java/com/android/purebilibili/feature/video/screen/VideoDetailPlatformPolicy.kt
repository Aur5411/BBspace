 ...
 * 视频页高刷选择。
 *
 * 现已是全局策略 [com.android.purebilibili.core.ui.performance.resolveHighRefreshModeId]
 * 的薄封装（保留此签名是为了兼容既有单测与调用点）：
 * 行为等价于「最高可用」档位 —— 同分辨率内挑 ≥ [minRefreshRate] 里最高的一档。
 */
internal fun resolvePreferredHighRefreshModeId(
    currentModeId: Int,
    supportedModes: List<RefreshModeCandidate>,
    minRefreshRate: Float = 90f
): Int? = com.android.purebilibili.core.ui.performance.resolveHighRefreshModeId(
    currentModeId = currentModeId,
    supportedModes = supportedModes.map { mode ->
        com.android.purebilibili.core.ui.performance.DisplayRefreshMode(
            modeId = mode.modeId,
    ...
    target = com.android.purebilibili.core.ui.performance.HighRefreshRateTarget.MAX,
