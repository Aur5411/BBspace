...
abstract class ExportAppApkTask : org.gradle.api.DefaultTask() {
    ...
    fun export() {
        ...
        check(
            deliveryName.startsWith("BBspace-") &&
            ...
            "Delivery APK must be BBspace-<version>.apk, not default AGP names like app-release.apk. Got: $deliveryName"
        }
        ...
        logger.lifecycle("BBspace delivery APK → ${target.absolutePath}")
    }
}
...
android {
    ...
    signingConfigs {
        create("release") {
            storeFile = file(System.getenv("BBSPACE_KEYSTORE") ?: "bbspace.keystore")
            storePassword = System.getenv("BBSPACE_STORE_PASS") ?: "bbspace2026"
            keyAlias = System.getenv("BBSPACE_KEY_ALIAS") ?: "bbspace"
            keyPassword = System.getenv("BBSPACE_KEY_PASS") ?: "bbspace2026"
...
base {
    archivesName.set("BBspace-$biliApkVersionName")
}
androidComponents {
    onVariants(selector().all()) { variant ->
        ...
        if (variantName == "release" || variantName == "dev") {
            ...
            val deliveryFileName = when (variantName) {
                else -> "BBspace-$biliApkVersionName.apk"
            }
            ...
            val exportTask = tasks.register<ExportAppApkTask>(
                ...
                outputDirectory.set(layout.buildDirectory.dir("outputs/bbspace/$variantName"))
            }
