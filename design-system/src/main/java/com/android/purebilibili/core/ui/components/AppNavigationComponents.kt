...
fun AppPlatformNavigationRail(
    ...
    // Miuix 0.9.4 起 NavigationRail 拆成两个重载：
    //   - NavigationRail(state: NavigationRailState, ...)  可展开/收起
    //   - NavigationRail(expanded: Boolean, ...)          固定宽度
    // 这里按 state 是否可用来分派，保留调用方的 null 语义。
    if (state == null) {
        MiuixNavigationRail(
            modifier = modifier.focusGroup(),
            expanded = false,
